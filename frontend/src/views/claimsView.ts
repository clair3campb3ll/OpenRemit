...
function renderEnrollCard(group: Group, user: User, summary: MembershipSummary): string {
  ...
  if (!user.walletAddress) {
    ...
    const prepayOptions = [1, 2, 3, 6]
      .map(n => {
        const amt = formatZAR(String(BigInt(summary.premiumMinor) * BigInt(n)), scale);
        return `<option value="${n}">${n} month${n > 1 ? 's' : ''} — ${amt}</option>`;
      })
      .join('');
    body = `
      <div>
        <span class="status-badge status-completed">Debit order active</span>
        <p class="muted" style="font-size:.85rem;margin:.6rem 0 .75rem">
        ...
        <div style="border-top:1px solid var(--border);padding-top:.75rem;margin-top:.25rem">
          <p class="muted" style="font-size:.82rem;margin:0 0 .5rem">Pay ahead in multiples of ${premium}:</p>
          <div style="display:flex;gap:.75rem;align-items:center;flex-wrap:wrap">
            <select id="prepay-months" class="input" style="width:auto">
              ${prepayOptions}
            </select>
            <button id="prepay-btn" class="btn btn-secondary">Pay now</button>
          </div>
          <span id="prepay-error" class="error-msg" style="display:none;margin-top:.5rem;font-size:.85rem"></span>
        </div>
      </div>`;
  } else {
...
export async function renderClaimsView(container: HTMLElement, currentUser: User): Promise<void> {
  ...
  try {
    [groups, allClaims] = await Promise.all([
      api.claims.groups(),
      api.claims.list(),
    ]);
  } catch (err) {
  ...
  // Prepay confirmation: N months paid in a lump sum.
  const prepayMonths = localStorage.getItem('fireline:prepay');
  if (prepayMonths) localStorage.removeItem('fireline:prepay');
  ...
  const prepayBanner = prepayMonths
    ? `<div class="success-msg" style="margin-bottom:1.5rem">Paid ${prepayMonths} month${Number(prepayMonths) > 1 ? 's' : ''} ahead — your next charge is pushed back accordingly.</div>`
    : '';
  ...
  // ── Lump-sum prepay ──────────────────────────────────────────────────────────
  const prepayBtn = container.querySelector<HTMLButtonElement>('#prepay-btn');
  if (prepayBtn) {
    const prepayErrEl  = container.querySelector<HTMLElement>('#prepay-error')!;
    const prepaySelect = container.querySelector<HTMLSelectElement>('#prepay-months')!;
    prepayBtn.addEventListener('click', async () => {
      prepayErrEl.style.display = 'none';
      prepayBtn.disabled        = true;
      const months = parseInt(prepaySelect.value, 10);
      prepayBtn.textContent = `Preparing ${months}-month payment…`;
      try {
        const { interactUrl } = await api.memberships.prepay(months);
        prepayBtn.textContent = 'Redirecting to wallet…';
        window.location.href  = interactUrl;
      } catch (err) {
        prepayErrEl.textContent   = String(err);
        prepayErrEl.style.display = 'block';
        prepayBtn.disabled        = false;
        prepayBtn.textContent     = 'Pay now';
      }
    });
  }
