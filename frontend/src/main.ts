...
async function route(): Promise<void> {
  ...
  if (returnId) {
    ...
    const prepayN  = params.get('prepay');
    if (prepayN && isLoggedIn()) {
      localStorage.setItem('fireline:prepay', prepayN);
      history.replaceState({}, '', window.location.pathname + '#/claims');
      // fall through to the hash router below
    } else if (payout && isLoggedIn()) {
