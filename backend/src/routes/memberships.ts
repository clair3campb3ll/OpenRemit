...
import { createEnrollment, runDueDebits, requestPrepay } from '../lib/debitOrder';
...
// ─────────────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// POST /api/memberships/prepay
// Pay N months' premium as a one-off lump sum (multiples of the monthly amount).
// Returns an interactUrl so the member can approve the grant at their wallet.
// Body: { months: number }  — integer 1–12
// ─────────────────────────────────────────────────────────────────────────────
membershipsRouter.post('/prepay', requireAuth, async (req, res, next) => {
  try {
    ...
    const [me] = await db.select().from(users).where(eq(users.id, req.user!.id));
    if (!me?.walletAddress) {
      return res.status(400).json({ error: 'Add a wallet address in your Profile before making a prepayment.' });
    }
    ...
    if (!membership) {
      return res.status(400).json({ error: 'You need an active debit order before making a prepayment.' });
    }

    const { interactUrl } = await requestPrepay(membership.id, months);
