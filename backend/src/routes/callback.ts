...
callbackRouter.get('/', async (req, res) => {
  ...
  const { interact_ref, transactionId, membershipId, prepayMonths, result } = req.query as Record<string, string>;
  ...
  // prepayMonths being set means this is a lump-sum prepay, not enrollment.
  if (membershipId && !prepayMonths) {
  ...
  try {
    ...
    // Lump-sum prepay: advance the membership schedule by N months.
    let prepaySuffix = '';
    if (membershipId && prepayMonths) {
      const months = parseInt(prepayMonths, 10);
      const [m] = await db.select().from(memberships).where(eq(memberships.id, membershipId));
      if (m?.status === 'ACTIVE') {
        const base = new Date(m.nextChargeAt ?? new Date());
        base.setMonth(base.getMonth() + months);
        await db.update(memberships)
          .set({ chargesMade: m.chargesMade + months, nextChargeAt: base, updatedAt: new Date() })
          .where(eq(memberships.id, m.id));
        console.log('[callback] Prepay: membershipId=%s advanced %d months → nextChargeAt=%s', m.id, months, base.toISOString());
      }
      prepaySuffix = `&prepay=${months}`;
    }

    res.redirect(`${config.frontendUrl}?status=completed&id=${transactionId}${postSuffix}${payoutSuffix}${prepaySuffix}`);
  } catch (err) {
    const message = (err as any)?.description ?? (err instanceof Error ? err.message : String(err));
