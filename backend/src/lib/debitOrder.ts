...
// Request a one-off GNAP grant for N months' premium in a single lump sum.
// Creates a quote (member → pool) for N × monthlyAmount, then requests an
// interactive outgoing-payment grant with no interval (so the per-P1M cap
// of the recurring grant doesn't constrain it). The callback advances
// nextChargeAt by N months on success.
export async function requestPrepay(
  membershipId: string,
  months: number,
): Promise<{ interactUrl: string }> {
  const [m] = await db.select().from(memberships).where(eq(memberships.id, membershipId));
  if (!m)                    throw new Error('Membership not found');
  if (m.status !== 'ACTIVE') throw new Error('Membership is not active');

  const lumpAmount = String(BigInt(m.monthlyAmount) * BigInt(months));
  ...
  const nonce        = crypto.randomUUID();
  const callbackUrl  =
    `${config.backendUrl}/api/callback` +
    `?transactionId=${flow.transactionId}&membershipId=${membershipId}&prepayMonths=${months}`;
  ...
  if (!isPendingGrant(grant) || !grant.interact?.redirect) {
    throw new Error('Expected interactive grant with redirect URL');
  }
  ...
  await db.update(transactions)
    ...
    .where(eq(transactions.id, flow.transactionId));

  console.log('[debit:prepay-requested] membershipId=%s months=%d lumpAmount=%s', membershipId, months, lumpAmount);
  return { interactUrl: grant.interact.redirect };
}
